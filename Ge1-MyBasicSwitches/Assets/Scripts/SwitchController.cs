...
public class SwitchController : MonoBehaviour {
	...
	public void turnOn() {
		switchOff = false;
		switchAnimator.SetBool("SwitchOff", switchOff);//switchOff used because false is set in the variable above (you can also write false for shorthand if u wish)
    }
	...
	public void toggleSwitch() {
		if (switchEnabled == true) {
			switchOff = !switchOff; //flipping it
			switchAnimator.SetBool ("SwitchOff", switchOff); 
		} else {
			Debug.Log ("Can't toggle switch as it's not enabled");//turning it to what it wasn't
        }
	}
}
